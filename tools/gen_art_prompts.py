...
def render(bible, records):
    ...
    w("4. **Fronta pod deset.** Jedenáctý souběžný job se vrátí *jako text, ne jako")
    w("   chyba* — tělo odpovědi se musí číst, jinak skript čeká na job, který nevznikl.")
    w("5. **Výsledky drží ~8 hodin.** Stáhni hned, `curl` na download URL")
    w("   (nekončí na `.png`), `Authorization: Bearer`. Žádný base64 do kontextu.")
    w("6. **Id se mezi voláním a vyzvednutím přejmenovává:** `get_image(job_id=…)`,")
    w("   `get_tiles_pro(tile_id=…)`, `get_character(character_id=…)`,")
    w("   `get_object(object_id=…)`. Stav, na který se čeká, je `creating`, ne")
    w("   `processing`.")
    w("7. **Po výměně PNG na disku `--headless --import`**, jinak hra tiše kreslí")
    w("   staré textury z cache.")
    w("8. **Objednávka může být větší než cíl.** U postav se generuje na dvojnásobku")
