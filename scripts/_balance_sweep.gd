...
## Three seeds, not one. A balance verdict read off a single seed is not a verdict: the
## spawn stream, the draft hand and every unseeded randf() in the run all move with it, and
## "this level is winnable" has to mean "winnable across the RNG", not "winnable on 20260829".
## Q2 already used three seeds by hand for exactly this reason (BLOCKED.md); this folds that
## discipline into the tool so nobody has to remember it again. The first entry stays the
## historical S3 seed so old numbers remain comparable.
const SEEDS := [20260829, 20260902, 20260903]
## The per-wave breakdown (§2) is emitted for this seed only. Three copies of it would
## triple the document's length to show the same shape three times; the outcome table above
## it already carries every seed, which is where a seed-dependent result would show up.
const DETAIL_SEED := 20260829
...
func _run() -> void:
	...
	rows.append("")
	rows.append("Generated by `scripts/_balance_sweep.gd` via S2's `LevelSimulator`. Section 1 runs")
	rows.append("every level × strategy on **each** of %d seeds (%s); a strategy that wins on one" % [
		SEEDS.size(), ", ".join(SEEDS.map(func(x): return str(x)))])
	rows.append("seed and loses on another is a level whose verdict depends on the RNG, which is")
	rows.append("itself the finding. Section 2's per-wave detail is emitted for seed %d only." % DETAIL_SEED)
	rows.append("")
	...
	rows.append("")
	rows.append("| level | strategy | seed | result | focus | max_focus | tolerance | dopamine | kills | wave |")
	rows.append("|---|---|---|---|---|---|---|---|---|---|")
	...
	for i in range(Data.get_level_count()):
		...
		for entry in STRATEGIES:
			...
			for run_seed: int in SEEDS:
				...
				print("-- level %d (%s) / %s / seed %d --" % [lv.id, lv.display_name, label, run_seed])
				var sim := LevelSimulator.new()
				add_child(sim)
				var r: Dictionary = await sim.run(lv.id, run_seed, recorder, MAX_FRAMES)
				recorder.finish(sim)
				sim.queue_free()
				await get_tree().process_frame
				print("   %s" % r)
				rows.append("| %s (id %d) | %s | %d | %s | %d | %d | %.1f | %d | %d | %d |" % [
					lv.display_name, lv.id, label, run_seed, _outcome_label(r),
					r.get("focus", -1), r.get("max_focus", -1), r.get("tolerance", -1.0),
					r.get("dopamine", -1), r.get("kills", -1), r.get("wave", -1)])

				if run_seed != DETAIL_SEED:
					continue
